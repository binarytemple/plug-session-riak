...
`Plug.Session.RIAK`, presumably in a Phoenix endpoint.
...
def application do
  [
    ...
    applications: [
        ...
       :lager, :corman # <-- add these both (corman is probably optional )
    ],
    included_applications: [
        :plug_session_riak # <--- add this entry
    ]
  ]
end
...
defp deps do
  [
    ...
    {:plug_session_riak, "~> 0.0.1" }, # <-- add this entry
    {:riak, "~> 1.0"}  # <-- and this one 
  ]
end
```
